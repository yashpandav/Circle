...
router.post('/create', auth, uploadLimiter, validateFiles, asyncHandler(createAss));

const { editAss } = require('../Controllers/AssignmentControllers/editAss');
router.put('/edit/:id', auth, uploadLimiter, validateFiles, asyncHandler(editAss));
...
router.post('/submit/:id', auth, uploadLimiter, validateFiles, asyncHandler(submitAss));
